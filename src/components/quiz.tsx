...
let searchTerms = "quizzes";
if (action === 'search' && mod === 'search') { searchTerms = `quizzes?title=${encodeURIComponent(q)}`; } 

const { data, isLoading, isFetching, refetch } = useQuery<ApiResponse<RowsResponse>>({
queryKey: ["quizzes", searchTerms],
enabled: !deleteId, // 🚫 don’t fetch if deleteId exists
queryFn: async () => {
const response = await fetchData<ApiResponse<RowsResponse>>(`${searchTerms}`, {}, 1000);
if (!response) throw new Error("No data returned from lessons endpoint");
return response;
},
staleTime: 15 * 1000,
refetchOnMount: true,
...
queryClient.setQueryData<ApiResponse<RowsResponse>>(["quizzes", searchTerms], (old) => {
if (!old) return old;
return {
...old,
data: {
...
{(action === 'search' && mod === 'search' && q !== "defaultTerm") ? <section>Search results: {q}</section> : null}
