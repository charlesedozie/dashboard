...
useEffect(() => {
    try {
      ...
      console.log("parsedOptions", parsed);
...
function handleSetval(){ console.log('handleSetval'); }
...
return (
<div className="p-4">
<SubTitle string1={'Update Question'} />{questionId} defVal:  {coalesce(avatar) ?? coalesce(curQuiz?.avatar) ?? coalesce(getFormFieldValue(formField, "avatar")) ?? ""},
...
<div>
{/* Dropdown Select */}
<label className='font-bold' htmlFor='quizQuestion'>Question Type</label> <span className="text-red-500"> *</span><br />rr {coalesce(formData.selectedValue) ?? coalesce(type) ?? ""} ee
<select
...
<p>Question Type : {formData.selectedValue}</p>
<p>Quiz question: {formData.textValue}</p>
{/*<p>Selected Option: {selected !== null ? texts[selected] : "None"}</p>*/}
<p>Selected Option: {selected !== null ? (texts[selected].title) : "None"}</p>
