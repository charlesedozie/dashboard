...
const exportToPDF = () => {
  ...
  return (
    <div className={`text-sm flex flex-wrap gap-2 items-center ${className}`}>
      {/* === Column Selector Dropdown === */}
      <div className="relative" ref={dropdownRef}>
        <button
          onClick={() => setOpen((prev) => !prev)}
          className="border border-gray-300 rounded-md px-4 py-2 bg-gray-50 hover:bg-gray-100 flex justify-between items-center w-full sm:w-auto"
        >
          <span>⚙️ Columns</span>
          <span className={`ml-2 transform transition-transform ${open ? "rotate-180" : "rotate-0"}`}>
            ▼
          </span>
        </button>

        {open && (
          <div className="absolute z-[9999] mt-2 bg-white border border-gray-200 rounded-md shadow-lg p-3 w-56">
      ...
      <button
        ...
        className="def-bg pointer text-sm text-white px-4 py-2 rounded-md hover:bg-blue-700"
      >
