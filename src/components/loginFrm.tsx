...
setReady(true); // allow rendering only if no redirect
}
}, [router]);
...
useEffect(() => {
const username = sessionStorage.getItem("username");
if (username) { router.replace("/user-area/dboard/"); }}, [router]);
*/
...
// Convert object into array entries
...
console.log(result.message);
console.log(result.data.user.username || result.data.user.email);
console.log(result);
//console.log(result.error);
// Parse the string into an object
//const obj = JSON.parse(result);
...
}} className="mb-1 def-color">Welcome Back</h2>
...
Email {sessionStorage.getItem("username") ? sessionStorage.getItem("username") : 'none'}
</label>
<input
type="email"
