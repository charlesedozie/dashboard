...
console.log('user id ', getUser()?.user.id)
...
console.log('current', OTP)
console.log('ENTERED', otpCode)
...
if(OTP == otpCode){
  ...
  try {
    console.log('getUser()?.user.id', getUser()?.user.id)
...
setError('');
reset();
        console.log('redirect to password update')
      router.push("/logout");
      }
    } catch (error) {
...
/*
const response = await fetch(`${process.env.NEXT_PUBLIC_OTP}`, {
method: 'POST',
headers: { 'Content-Type': 'application/json' },
body: JSON.stringify({ otp: otpCode, email:getUser()?.user.email || '', id:'N9HnMguA9-x3oXjUZn4cZ91_M8DLoGc1OVgwh9yJxI8', }),
});
...
// Simulate resending OTP via your backend
 await postForm(`${process.env.NEXT_PUBLIC_OTP}`, { otp: OTP || '', email:getUser()?.user.email || '', id:'N9HnMguA9-x3oXjUZn4cZ91_M8DLoGc1OVgwh9yJxI8', }, 'pr=9w&2D2');

 /*
const response = await fetch(`${process.env.NEXT_PUBLIC_OTP}`, {
method: 'POST',
headers: { 'Content-Type': 'application/json' },
body: JSON.stringify({ otp: OTP, email:getUser()?.user.email || '', id:'N9HnMguA9-x3oXjUZn4cZ91_M8DLoGc1OVgwh9yJxI8', }), // Send current OTP or user email/ID if needed
});
