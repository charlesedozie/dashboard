...
export default function Tutors() {
  ...
  return (
          ...
          <table className="w-full border-collapse text-gray-500">
            <thead className="bg-gray-50 sticky top-0 z-10">
              <tr>
                ...
                <th className="py-2 text-left text-sm border-b border-gray-200">Tutor</th>
                ...
                <th className="py-2 text-center text-sm border-b border-gray-200">Remove</th>
              </tr>
            </thead>
            <tbody>
              {currentTutors.map((tutor, index) => (
                <tr key={tutor.id || index} className="border-b border-gray-200">
                  ...
                  <td className="py-3 text-sm align-top">{tutor.fullName || "N/A"}</td>
                  ...
                  <td className="py-3 text-sm align-top">
                    <DropDownAPI
                      ...
                      options={UserStatus}
                      defaultState={tutor.status || "Active"}
    ...
    <ul className="list-none text-sm p-0 m-0 flex flex-col items-start justify-start">
      {tutor.subjects.map((subject: { id?: string | number; title?: string } | string, idx: number) =>
    ...
    <p>N/A</p>
      ...
      {deleteTutor && (
        <Delete
          apiUrl={`/api/delete-tutor/${deleteTutor.id}`}
          title={`Delete Tutor: ${deleteTutor.fullName}`}
