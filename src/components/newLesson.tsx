...
//console.log('curLesson', data)
}
}, [data]);

//if(itemId !== null && itemId !== '' && lessonId !== itemId){setLessonId(itemId);}
...
// Add files / images
/*
const addFileImage = (newItem: FilesImages) => { 
const nameExists = filesImages.some(
existingItem => existingItem.name.toLowerCase() === newItem.name.toLowerCase());
if (nameExists) { console.log(`"${newItem.name}" already exists!`); return; }
setFilesImages(prevItems => [...prevItems, newItem]); };
*/
function updateFileImage(status: string, msg: string): void {
console.log('updateFileImage: ', msg); handleAddByName(msg);}
...
if (!exists) { console.log(`"${id}" not found in the list!`); return prevItems; }

console.log('prevItems.length', prevItems.length)
if (prevItems.length < 2) {
console.log("Only one item left — clearing array.");
return [];
}
...
console.log("Only one item left — clearing array. 999999");
setFilesImages([]);
return [];
}
console.log("Removed:", id, "Remaining:", newItems);
return newItems;
});
};
...
useEffect(() => { if (curTopicData) { setCurTopic(curTopicData); console.log('cur curTopicData', curTopicData)   }}, [curTopicData]);
...
if (lessonId && lessonId.id !== "") {
//router.push(`/user-area/lessons?action=update&mod=lesson&itemId=${lessonId.id}`);
}
}, [lessonId, router]);
...
required: true,
className: 'w-full',
defVal: coalesce(videoCaption) ?? coalesce(getFormFieldValue(videoField, "avatarOrCover")) ?? "",
handleImage:updateVideoCaption,
},
{
name: 'lessonId',
label: String(itemId),
...
<th className="py-2 text-left text-sm border-b border-gray-200">Title</th>
<th className="py-2 text-left text-sm border-b border-gray-200">Type</th>
<th className="py-2 text-left text-sm border-b border-gray-200">Duration</th>
...
<td className="py-3 text-sm align-top">{tutor.duration || "N/A"}</td>
...
title={`Delete Quiz: ${curTopicTitle ?? ""}`}
message="Are you sure you want to proceed?"
cancelLabel="Cancel"
confirmLabel="Confirm"
isOpen={!!deleteTopic}
