...
function getUserFullName(userId: string, users?: any[]): string {
  ...
  if (!user) {
    ...
    return "Unknown User 22";
  }

  return user.fullName || user.username || "Unnamed User";
}
...
<p className="text-gray-500 text-sm">{getUserFullName(quizz.userId ?? "", userNames)}</p>
</div>
</div>
</div>
))}
...
{/* Pagination */}
<div className="flex justify-between items-center mt-14 text-sm text-gray-600">
<div>
