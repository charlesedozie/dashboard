...
interface MultiSelectProps {
  options: Option[];
  redirectUrl?: string; // ✅ new prop for base redirect URL
}
...
export default function MultiSelectDropdown({ options, redirectUrl }: MultiSelectProps) {
  ...
  const handleApply = () => {
    ...
    const redirectWithParams = `${redirectUrl}&subjects=${params}`; // ✅ build URL dynamically
      ...
      {isOpen && (
        <div className="absolute mt-1 w-full border bg-white shadow-md rounded-md z-10 p-2">
          ...
          <button
            ...
            className={`mt-3 w-full p-2 rounded text-white pointer ${
